        ...
        const sendMessage = (retryCount = 0) => {
            try {
                ...
                if (messagePromise && typeof messagePromise.then === 'function') {
                    ...
                    browserAPI.tabs.sendMessage(tabId, { action }, (response: any) => {
                        if (browserAPI.runtime.lastError) {
...
async function injectContentScript(tabId: number, action: Actions): Promise<void> {
    try {
        ...
        browserAPI.tabs.sendMessage(tabId, { action }).catch((error: any) => {
        ...
        try {
            ...
            browserAPI.tabs.sendMessage(tabId, { action }).catch((error2: any) => {
